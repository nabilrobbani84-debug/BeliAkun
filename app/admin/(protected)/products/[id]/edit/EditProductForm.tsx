...
export default function EditProductForm({
  ...
  const [badge, setBadge] = useState<'none' | 'bestseller' | 'saving' | 'new' | 'limited_stock'>(product.badge as any)
  const [deliveryMethod, setDeliveryMethod] = useState<'instant' | 'manual'>(product.delivery_method as any)
  const [warrantyEnabled, setWarrantyEnabled] = useState(product.warranty_enabled)
  const [warrantyDuration, setWarrantyDuration] = useState<number | null>(product.warranty_duration || 7)
  const [warrantyUnit, setWarrantyUnit] = useState<'day' | 'week' | 'month' | 'year' | 'lifetime' | 'custom' | null>(product.warranty_unit as any || 'day')
  const [warrantyLabel, setWarrantyLabel] = useState(product.warranty_label || '7 Hari Garansi')
  const [status, setStatus] = useState<'draft' | 'active' | 'inactive' | 'archived'>(product.status as any)
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-center gap-4">
        ...
        <div>
          <h1 className="font-black text-2xl sm:text-3xl text-slate-900">Ubah Produk</h1>
          <p className="font-semibold text-slate-500 text-xs sm:text-sm">
            Ubah rincian informasi dasar untuk {product.name}.
          </p>
        </div>
      </div>
      ...
      <form onSubmit={handleSubmit} className="space-y-6 max-w-4xl">
              ...
              <div key={idx} className="flex gap-2 items-center">
                ...
                {features.length > 1 && (
                  <button
                    ...
                    className="p-2 border-2 border-red-955 bg-red-50 text-red-700 rounded-xl"
                  >
        ...
        <div className="cartoon-card p-6 bg-white border-3 border-slate-900 rounded-2xl shadow-[4px_4px_0px_0px_#0F172A] space-y-4">
          ...
          {warrantyEnabled && (
            <div className="p-4 bg-slate-50 border-2 border-slate-900 rounded-xl grid grid-cols-1 sm:grid-cols-3 gap-4">
              <div className="space-y-1">
        ...
        <div className="cartoon-card p-6 bg-white border-3 border-slate-900 rounded-2xl shadow-[4px_4px_0px_0px_#0F172A] space-y-4">
          ...
          <div className="pt-4 border-t-2 border-slate-100 flex justify-end gap-3">
            ...
            >
              <Save className="w-4 h-4" /> {isSubmitting ? 'Memproses...' : 'Perbarui Produk'}
            </button>
          </div>
        </div>
      </form>
